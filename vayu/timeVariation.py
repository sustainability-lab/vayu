def timeVariation(df, pollutant):
    ...
    for i in range (len(dayWeek)) : 
      ...
      for k in range (len(pollutant)):    
        ...
        plt.fill_between(df_day_s['hour'], df_day_m[pollutant[k]]-0.5*df_day_s[pollutant[k]], 
                         df_day_m[pollutant[k]]+0.5*df_day_s[pollutant[k]],alpha=0.2)
    ...
    for i in range (len(pollutant)):
      ...
      plt.fill_between(df_hour_s['hour'], df_hour_m[pollutant[i]]-0.5*df_hour_s[pollutant[i]], 
                         df_hour_m[pollutant[i]]+0.5*df_hour_s[pollutant[i]], alpha=0.2)
      plt.xlabel("Hour")
      plt.legend()
    ...
    for i in range (len(pollutant)):
      ...
      plt.fill_between(df_month_s['month'], df_month_m[pollutant[i]]-0.5*df_month_s[pollutant[i]], 
                       df_month_m[pollutant[i]]+0.5*df_month_s[pollutant[i]], alpha=0.2)
      plt.xlabel("Month")
      plt.legend()
    ...
    for i in range (len(pollutant)):
      plt.plot(df_weekday_m['weekday'], df_weekday_m[pollutant[i]],label = pollutant[i])
      plt.fill_between(df_weekday_s['weekday'], df_weekday_m[pollutant[i]]-0.5*df_weekday_s[pollutant[i]], 
                       df_weekday_m[pollutant[i]]+0.5*df_weekday_s[pollutant[i]], alpha=0.2)
      plt.xlabel("WeekDay")
      plt.legend()
